...
// 	// client, err := rpc.Dial("tcp", "localhost:1234")	// 使用基于tcp的rpc
// 	// client, err := rpc.DialHTTP("tcp", "localhost:1234") // 使用基于http的rpc
// 	client, err := jsonrpc.Dial("tcp", "localhost:1234") // 使用基于tcp的json rpc
...
//基于http的json rpc客户端
// 通过建立好的TCP 链接，发送 HTTP 请求调用远程的HTTP JSON RPC 服务，这里使用的是 HTTP GET 方法
func main() {
